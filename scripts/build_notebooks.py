...
> Procediamo per step. Questo e lo **Step 1: caricamento e panoramica strutturale**.""",
        ...
        "s7_tp_intro": "## 7 · Reale vs predetto al variare del numero di GPU (TP), concurrency fissa\n\nDomanda: *dove* si concentra l'errore del modello fisico al crescere del parallelismo, e il correttivo ML lo assorbe? A concurrency fissa (C=1, il regime più pulito: niente coda) due viste complementari: **(a) valori assoluti** — benchmark reale contro roofline/targa contro ML (predizioni out-of-fold LOMO) — separati per piattaforma (**H100 PCIe** sopra, **H200 NVLink** sotto) su un modello rappresentativo con sweep TP completo su entrambe (Qwen2.5-32B); **(b) errore mediano aggregato** per livello di TP su tutti i modelli. Nota chiave: a TP=1 non c'è comunicazione inter-GPU (η=1), quindi l'errore residuo lì misura la pura qualità di MFU/MBU; da TP≥2 entra in gioco il termine η.",
        "s7_tp_end": "### Lettura\n\n**Vista a valori (per piattaforma):**\n- **Potenza**: la prova visiva della sub-linearità — su H100 PCIe il consumo reale passa da ~417 a ~658 W tra TP=1 e TP=8 mentre la targa sale linearmente da 350 a 2800 W; su H200 ~995 W reali contro 5600 di targa a TP=8. La curva ML (out-of-fold) è **sovrapposta al reale** su tutta la griglia, su entrambe le piattaforme.\n- **Latenze**: le misure mostrano varianza per-configurazione che nessuna curva liscia cattura (es. il TTFT reale su H200 a TP=1); l'ML segue la *forma* del reale meglio del roofline su entrambe le piattaforme, ma non punto per punto.\n\n**Vista aggregata (medAPE per TP):**\n- **TTFT**: a TP=1 il roofline è quasi esatto (~8%) e l'ML *non* lo migliora (~16%): dove la fisica basta, il correttivo non ha nulla da correggere. Da TP≥2 l'errore fisico sale (~40–60%) e il vantaggio ML cresce con il parallelismo, fino a ~3× a TP=8 (22% vs 62%).\n- **ITL**: il roofline ha il suo regime peggiore a TP=2 (~160%); l'ML lo tiene ≤34% a ogni TP.\n- **Potenza**: l'errore della targa **cresce monotonicamente con il TP** (11% → 331%); l'ML resta ≤2% ovunque.\n- Robustezza: alle altre concurrency il pattern si conserva — il vantaggio ML si concentra ai TP alti.\n\n**Messaggio**: l'errore del modello fisico non è distribuito uniformemente — si accumula dove entrano la comunicazione multi-GPU e la sub-linearità della potenza. Il correttivo ML impara esattamente questi regimi ed è lì che serve; a TP=1 la fisica calibrata è già sufficiente.",
...
> We proceed step by step. This is **Step 1: loading and structural overview**.""",
        ...
        "s7_tp_intro": "## 7 · Real vs predicted vs GPU count (TP) at fixed concurrency\n\nQuestion: *where* does the physical model's error concentrate as parallelism grows, and does the ML corrector absorb it? At fixed concurrency (C=1, the cleanest regime: no queueing), two complementary views: **(a) absolute values** — real benchmark vs roofline/nameplate vs ML (out-of-fold LOMO predictions) — split by platform (**H100 PCIe** top, **H200 NVLink** bottom) for a representative model with a full TP sweep on both (Qwen2.5-32B); **(b) aggregate median error** per TP level across all models. Key note: at TP=1 there is no inter-GPU communication (η=1), so the residual error there measures pure MFU/MBU quality; from TP≥2 the η term kicks in.",
        "s7_tp_end": "### Reading\n\n**Value view (per platform):**\n- **Power**: the visual proof of sub-linearity — on H100 PCIe real consumption goes from ~417 to ~658 W between TP=1 and TP=8 while the nameplate climbs linearly from 350 to 2800 W; on H200, ~995 W real vs 5600 nameplate at TP=8. The ML curve (out-of-fold) **overlaps the real one** across the whole grid, on both platforms.\n- **Latencies**: the measurements show per-configuration variance that no smooth curve captures (e.g. the real H200 TTFT at TP=1); ML follows the *shape* of the real data better than the roofline on both platforms, but not point by point.\n\n**Aggregate view (medAPE per TP):**\n- **TTFT**: at TP=1 the roofline is nearly exact (~8%) and ML does *not* improve it (~16%): where the physics suffices, the corrector has nothing to correct. From TP≥2 the physical error rises (~40–60%) and the ML advantage grows with parallelism, up to ~3× at TP=8 (22% vs 62%).\n- **ITL**: the roofline has its worst regime at TP=2 (~160%); ML keeps it ≤34% at every TP.\n- **Power**: the nameplate error **grows monotonically with TP** (11% → 331%); ML stays ≤2% everywhere.\n- Robustness: at the other concurrency levels the pattern holds — the ML advantage concentrates at high TP.\n\n**Message**: the physical model's error is not uniformly distributed — it accumulates where multi-GPU communication and power sub-linearity enter. The ML corrector learns exactly these regimes, and that is where it is needed; at TP=1 the calibrated physics is already sufficient.",
...
CODE_ERR_TP = r'''# Section 7 - Real vs predicted vs GPU count (TP) at fixed concurrency.
# Two views: (a) absolute values - real benchmark vs roofline/nameplate vs ML,
# split by platform (H100 PCIe / H200 NVLink) for one representative model;
# (b) aggregate median error by TP across all models. Per-config LOMO
# predictions, best estimator per target (Step 5.3). TP=1 has no inter-GPU
# communication (eta=1): the residual error there measures pure MFU/MBU quality.
try:
...
# --- (a) absolute values by platform, representative model (full TP sweep on both)
MODEL_SHOW = "Qwen2.5-32B-Instruct"
COLTITLE = {"ttft_avg_ms": "TTFT (ms)", "itl_avg_ms": "ITL (ms)", "power_w": "power (W)"}
fig, axes = plt.subplots(2, 3, figsize=(13, 6), sharex="col")
for j, (tgt, (est, fr)) in enumerate(BEST_TP.items()):
    for i, (arch, archlbl) in enumerate([("H100", "H100 PCIe"), ("H200", "H200 NVLink")]):
        s = ep[(ep.model == MODEL_SHOW) & (ep.target == tgt) & (ep.estimator == est)
               & (ep.framing == fr) & (ep.concurrent_users == C_FIX)
               & (ep.gpu_arch == arch)].sort_values("n_gpu")
        ax = axes[i, j]
        x = s.n_gpu.astype(str)
        lbl_theo = "TDP nameplate" if tgt == "power_w" else "calibrated roofline"
        ax.plot(x, s.y_real, "o-", color="#333333", lw=2.4, ms=7, label="real benchmark", zorder=3)
        ax.plot(x, s.y_theo, "s--", color="#C44E52", label=lbl_theo)
        ax.plot(x, s.y_pred, "X-.", color="#55A868", ms=7, label="ML corrector (LOMO)", zorder=4)
        if i == 0: ax.set_title(COLTITLE[tgt])
        if j == 0: ax.set_ylabel(archlbl, fontsize=11, fontweight="bold")
        if i == 1: ax.set_xlabel("n GPU (= TP)")
        ax.grid(alpha=0.3)
h, _ = axes[0, 0].get_legend_handles_labels()
fig.legend(h, ["real benchmark", "roofline / nameplate", "ML corrector (LOMO, out-of-fold)"],
           loc="lower center", ncols=3, bbox_to_anchor=(0.5, -0.04), fontsize=10)
fig.suptitle(f"Real vs predicted by GPU count - {MODEL_SHOW}, C={C_FIX} (ML = out-of-fold)")
fig.tight_layout()
fig.savefig("results/figures/real_vs_pred_by_tp.png", dpi=150, bbox_inches="tight")
plt.show()

# --- (b) aggregate median error by TP across all models
